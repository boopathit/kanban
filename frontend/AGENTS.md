...
Next.js 16 (App Router) + React 19 + Tailwind CSS v4. Static export served by FastAPI; cookie-based auth; board state lives in SQLite via `/api/board` (Part 7).
...
frontend/
  ...
  src/
    app/
      ...
      page.tsx               "use client". Auth-gates the board, then wires `useBoard()` and feeds the result into <KanbanBoard>.
    ...
    components/
      KanbanBoard.tsx        "use client". **Presentational** — props: { board, loading, error, onDismissError, actions, onLogout? }. Renders [data-testid="board-loading"] while loading, otherwise the full Kanban + DragOverlay + Toast.
    ...
    lib/
      ...
      useBoard.ts            React hook owning BoardData. Loads on mount; exposes { board, loading, error, dismissError, reload, actions }. Optimistic create-after-server-id, move/delete/rename rollback on error; rename PATCH debounced 350 ms.
      useBoard.test.tsx      Vitest: load happy/error, createCard happy + 500 rollback, moveCard happy + rollback, deleteCard happy, debounce + rollback rename, dismissError.
  ...
  tests/
    ...
    persistence.spec.ts      Playwright: add card → reload → still there → delete → reload → gone; rename column → reload → name persists.
```
...
- Returns `dismissError()` for the toast's close button and `reload()` for manual refresh.
